...
exports['parser_stream'] = {
  ...
  'endOfStream for empty stream': function(test) {
    ...
    test.ok(stream.ofParser(p, stream.ofCharacters("")).endOfStream(0), 
  ...
  'endOfStream for non empty stream': function(test) {
    ...
    // tests here  
    var p = parser.char(' ').optrep().thenRight(parser.numberLiteral());
    test.ok(stream.ofParser(p, stream.ofCharacters("1")).endOfStream(1), 
            'should be endOfStream.');
    test.done();
  },
  ...
  'no endOfStream for non empty stream': function(test) {
    ...
    // tests here  
    var p = parser.char(' ').optrep().thenRight(parser.numberLiteral());
    test.equal(stream.ofParser(p, stream.ofCharacters("1")).endOfStream(0),
               false,
               'should be endOfStream.');
    test.done();
  },
  ...
  'get from stream': function(test) {
    ...
    // tests here  
    var p = parser.char(' ').optrep().thenRight(parser.numberLiteral());
    test.equal(stream.ofParser(p, stream.ofCharacters("1")).get(0).isSuccess(), 
               true,
               'should be a success.');
    test.done();
  },
  ...
  'do not get from empty stream': function(test) {
    ...
    // tests here  
    var p = parser.char(' ').optrep().thenRight(parser.numberLiteral());
    test.equal(stream.ofParser(p, stream.ofCharacters("1")).get(1).isSuccess(), 
               false,
               'should be a failure.');
    test.done();
  },
