...
export default {
  ...
  'expect (space) to be accepted': function(test) {
    ...
    // tests here
    test.equal(
      genlex.generator().space().parse(stream.ofString(' '), 0).isAccepted(),
      true,
      'should be accepted.'
    );
    test.done();
  ...
  'expect (ident) to return smith': function(test) {
    ...
    test.equal(
      genlex.generator([]).ident(factory).parse(stream.ofString('smith '), 0)
        .value,
      'smith',
      'should be smith.'
    );
  ...
  'expect (string) to return smith': function(test) {
    ...
    test.equal(
      genlex.generator([]).string(factory).parse(stream.ofString('"smith"'), 0)
        .value,
      'smith',
      'should be smith.'
    );
  ...
  'expect (char) to return j': function(test) {
    ...
    test.equal(
      genlex.generator([]).char(factory).parse(stream.ofString("'s'"), 0).value,
      's',
      'should be s.'
    );
    test.done();
  ...
  'expect (token) to return keyword': function(test) {
    ...
    test.equal(
      genlex.generator(['<-']).token(factory).parse(stream.ofString('a<-b'), 1)
        .value,
      '<-',
      'should be <-.'
    );
  ...
  'expect (token) to return ident': function(test) {
    ...
    test.equal(
      genlex.generator(['<-']).token(factory).parse(stream.ofString('hello'), 0)
        .value,
      'hello',
      'should be hello.'
    );
  ...
  'expect (token) to return number': function(test) {
    ...
    test.equal(
      genlex.generator(['<-']).token(factory).parse(stream.ofString('123'), 0)
        .value,
      123,
      'should be 123.'
    );
  ...
  'expect (token) to return char': function(test) {
    ...
    test.equal(
      genlex.generator(['<-']).token(factory).parse(stream.ofString("'a'"), 0)
        .value,
      'a',
      'should be a.'
    );
