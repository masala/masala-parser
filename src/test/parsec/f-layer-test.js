...
export default {
    ...
    'expect F.layer(parser) to work as parser with backtrak on success': function(
        test,
    ) {
    ...
    'expect F.layer(parser).and(other) to not move on the second after first fails': function(
        test,
    ) {
        const first = C.char('a')
            .then(C.char('a'))
