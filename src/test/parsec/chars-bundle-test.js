...
export default  {
    ...
    'expect accent to be accepted': function (test) {
        test.expect(1);
        // tests here
        test.equal(C.utf8Letter.parse(stream.ofString("é"), 0).isAccepted(),
            true,
            'should be accepted.');
        test.done();
        ...
        //Белград
        //български
        test.equal(C.utf8Letter.parse(stream.ofString("Б"), 0).isAccepted(),
            true,
            'should be accepted.');
        test.equal(C.utf8Letter.parse(stream.ofString("б"), 0).isAccepted(),
            true,
        ...
        test.expect(1);
        // tests here
        test.equal(C.utf8Letter.parse(stream.ofString("-"), 0).isAccepted(),
            false,
            'should be rejected.');
        test.done();
    }
