...
describe('Flow Bundle Tests', () => {
    it('subStream is ok on string stream', () => {
        ...
        const response = parser.parse(Streams.ofChar(text))
    ...
    it('not parser should not eat offset', () => {
        ...
        let response = parser.parse(Streams.ofChar(line))
        expect(response.isAccepted()).toBe(true)
        expect(response.offset).toBe(text.length)

        const withParser = F.not(eol).rep().then(eol)
        response = withParser.parse(Streams.ofChar(line))
        expect(response.isAccepted()).toBe(true)
    ...
    it('test moveUntilFast string', () => {
        const line = Streams.ofChar('soXYZso')
    ...
    it('test moveUntil', () => {
        const line = Streams.ofChar('I write until James appears')
    ...
    it('test moveUntil parser, with eos, not including', () => {
        const line = Streams.ofChar('I write until the end')
    ...
    it('lazy with a class', () => {
        ...
        const line = Streams.ofChar('ababa')
