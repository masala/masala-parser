...
export default {
    ...
    '1 paragraph bloc': function (test) {
        const block = `\nLorem ipsum is a first paragraph\n`;
        test.expect(1);
        // tests here

        const result = markdown.parse(stream.ofString(block), 0).value;
        test.deepEqual(
            result,
            {paragraph: {text: 'Lorem ipsum is a first paragraph'}},
            'should be a two paragraph.');
        test.done();
    }/*,

    '2 paragraph blocs are children of a title': function (test) {

        const block = `# The title

     Lorem ipsum is a common paragraph
     ...
     `;
        ...
        test.expect(1);
        // tests here
        test.deepEqual(markdown.parse(stream.ofString(block), 0).value,
            [
                {
                    title: [{text: "Title"}],
                    level: 2,
                    children: [
                        {paragraph: 'Lorem ipsum is a common paragraph'},
                        {paragraph: 'Lorem ipsum is a common paragraph'}
                    ]
                }
